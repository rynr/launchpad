...
public class MidiReaderTest {
    ...
    @Test
    public void testHandlesTwoByteCommand() throws InterruptedException,
            IOException {
        when(device.read()).thenReturn(256 + Command.NOTE_ON.getByte())
    ...
    @Test
    public void testHandlesOneByteCommand() throws InterruptedException,
            IOException {
        when(device.read()).thenReturn(256 + Command.PROGRAM_CHANGE.getByte())
                .thenReturn(4);
        midiReader.addHandler(handler);
        thread.start();
        Thread.sleep(DEFAULT_TEST_SLEEP_IN_MS);
    ...
    public void testHandlesSysEx() throws InterruptedException, IOException {
        when(device.read()).thenReturn(256 + Command.SYS_EX.getByte())
                .thenReturn(4).thenReturn(1).thenReturn(2).thenReturn(3)
                .thenReturn(4);
        midiReader.addHandler(handler);
        thread.start();
        Thread.sleep(DEFAULT_TEST_SLEEP_IN_MS);
