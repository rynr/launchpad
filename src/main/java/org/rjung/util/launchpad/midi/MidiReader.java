...
/**
 * The {@link MidiReader} listens to all events of the Launchpad and sends the information to all
 * registered {@link LaunchpadHandler}s.
 */
public class MidiReader implements Runnable, Closeable {
  ...
  /**
   * Create a {@link MidiReader} with the {@link RandomAccessFile} to the midi-device.
   *
   * @param device {@link RandomAccessFile} to the midi-device of the Launchpad.
   */
  ...
  /**
   * Add a {@link LaunchpadHandler} to the {@link MidiReader} to be informed of any events from the
   * Launchpad.
   *
   * @param handler The {@link LaunchpadHandler} to be added.
  ...
  /**
   * Remove a {@link LaunchpadHandler} to not be informed any more of any events from the Launchpad.
   *
   * @param handler The {@link LaunchpadHandler} to be removed.
  ...
  private byte[] getDataForCommand(final byte command) throws IOException {
    byte length = isSysEx(command) ? device.readByte() : takesBytesOfData(command);
  ...
  private byte takesBytesOfData(final byte command) {
    return (byte) (command >= (byte) 0xc0 && command < (byte) 0xe0 ? 1 : 2);
  }
